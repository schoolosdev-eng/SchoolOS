...
function getStripePixPriceId(planName: string, billingCycle?: string) {
  ...
  try {
    ...
    if (!schoolId || !planId || !planName || price === undefined) {
      return NextResponse.json(
        { error: 'Dados incompletos para gerar pagamento Pix.' },
        { status: 400 }
      )
    }

    const priceId = getStripePixPriceId(planName, billingCycle)

    if (!priceId) {
      return NextResponse.json(
        { error: 'Preço Pix Stripe não configurado para este plano.' },
        { status: 400 }
      )
    }
    ...
    if (paymentError || !paymentRow) {
      return NextResponse.json(
        {
          error: paymentError?.message || 'Erro ao criar pagamento Pix.',
    ...
    console.error('ERRO DETALHADO CHECKOUT PIX STRIPE:', error)
